...
query = es.search(index="sohuhao_all", doc_type="sohuhao",
...
for item in results:
    ...
    author_id = item['_source']['title']
    author_type = 'no_data'
    is_added = 0
    website = 'sohuhao'
    column = f"('{author_url}', '{author_id}', '{author_type}', {is_added}, '{website}')"
    column_list.append(column)

values = ",".join(column_list)
insert_column = f"insert ignore into author_other(author_url, author_id, author_type, is_added, website) values{values};"
print(insert_column)
